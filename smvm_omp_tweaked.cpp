...
int main() {
	...
	for (trial_ctr = 0; trial_ctr < trial_max; trial_ctr++)
	{
		...
		int i, j, ckey;
#pragma omp for private(ckey, j) schedule(dynamic) // change it to dynamic
		for (i = 0; i<126464; i++)
		{
			fp_ones[i] = 0;
	...
	cout << "Sparse matrix vector multiplication | OMP nontweaked v1 | forward projection of ones | Average time = " << avg_time << endl;
	system("pause");
	return 0;
}